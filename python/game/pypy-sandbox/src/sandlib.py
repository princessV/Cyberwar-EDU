...
import sys, os, posixpath, errno, stat, time, asyncio_interface
import subprocess
from rpython.tool.killsubprocess import killsubprocess
from rpython.translator.sandbox.vfs import UID, GID, RealDir
import py, socket

from virtualfileio import WriteableRealFile, ProtocolSocketWrapper
...
class VirtualizedSandboxedProc(SimpleIOSandboxedProc):
    ...
    def path_safe_for_writing(self, path):
        for safe_path in self.safe_write_paths:
            if path.startswith(safe_path): return True
        return False

    def get_node(self, vpath, canCreate=False):
        # assume path can be converted
        vpath = vpath.decode()
        dirnode, name = self.translate_path(vpath)
        if name:
            if isinstance(dirnode, RealDir):
                realPath = os.path.join(dirnode.path, name)
            else:
                realPath = None
            if canCreate and realPath is not None and self.path_safe_for_writing(vpath):
                node = WriteableRealFile(realPath)
            else:
                # otherwise, let the existing sandbox code decide if it's safe to open
    ...
    def do_ll_os__ll_os_open(self, vpathname, flags, mode):        
        if WriteableRealFile.getOpenModeString(flags) in ["rb"]:
            node = self.get_node(vpathname)
            f = node.open()
        else:
            node = self.get_node(vpathname, canCreate=True)
            if isinstance(node, WriteableRealFile):
...
class VirtualizedNetworkProc(VirtualizedSandboxedProc):
    ...
    def do_ll_os__ll_os_open(self, name, flags, mode):
        if name.startswith(b"game://"):
            ...
            connector, hostport = name.split(b"://")
            host, port = hostport.split(b":")
            
            connector = connector.decode()
            host = host.decode()
            port = int(port)
            # open("rip://20.20.20.20:8000")
            try:
                protocol = asyncio_interface.playground_connect(host, port, connector)
            except Exception as e:
                print("Exception on playground connection: {}".format(e))
                raise RuntimeError("Could not open connection to playground because {}".format(str(e)))
            fd = self.allocate_fd(protocol, ProtocolSocketWrapper())
            self.sockets[fd] = True
            return fd
        else:
            return super().do_ll_os__ll_os_open(name, flags, mode)
            

    def do_ll_os__ll_os_read(self, fd, size):
        if fd in self.sockets:
            # ignore size for network reading.
            try:
                data = self.get_file(fd).recv(size)
                return data
            except Exception as e:
                print("Read Error",e)
